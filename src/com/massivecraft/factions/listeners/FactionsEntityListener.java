...
public class FactionsEntityListener implements Listener {
    private static final Set<Integer> badPotionEffects = new LinkedHashSet<Integer>(Arrays.asList(
            Effect.BLINDNESS, Effect.CONFUSION, Effect.HARMING, Effect.HUNGER,
            Effect.POISON, Effect.SLOWNESS, Effect.MINING_FATIGUE, Effect.WEAKNESS,
            Effect.WITHER
    ));
    public P p;
    ...
    public void onEntityDeath(EntityDeathEvent event) {
        ...
        } else if (faction.isNone() && !Conf.wildernessPowerLoss && !Conf.worldsNoWildernessProtection.contains(player.getLevel().getName())) {
...
//		}
        if
                (
                (boomer instanceof EntityPrimedTNT /*|| boomer instanceof ExplosiveMin*/)
                        &&
                        (
                                (faction.isNone() && Conf.wildernessBlockTNT && !Conf.worldsNoWildernessProtection.contains(loc.getLevel().getName()))
                                        ||
            ...
            event.setCancelled(true);
        } else if ((boomer instanceof EntityPrimedTNT /*|| boomer instanceof ExplosiveMinecart*/) && Conf.handleExploitTNTWaterlog) {
...
//						target.breakNaturally();
                        target.getLevel().setBlockIdAt(target.getFloorX(), target.getFloorY(), target.getFloorZ(), Block.AIR);
    ...
    public void onEntityCombustByEntity(EntityCombustByEntityEvent event) {
        ...
        EntityDamageByEntityEvent sub = new EntityDamageByEntityEvent(event.getCombuster(), event.getEntity(), EntityDamageEvent.CAUSE_FIRE, 0);
...
//        }
//
//        if (Conf.safeZoneNerfedCreatureTypes.contains(event.getType()) && Board.getFactionAt(new FLocation(event.getPosition())).noMonstersInTerritory()) {
//            event.setCancelled(true);
//        }
//    }
