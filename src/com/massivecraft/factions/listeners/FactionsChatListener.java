    ...
    public void onPlayerEarlyChat(PlayerChatEvent event) {
        ...
        if (chat != ChatMode.PUBLIC && p.handleCommand(talkingPlayer, msg, false, true)) {
            if (Conf.logPlayerCommands)
                Server.getInstance().getLogger().log(LogLevel.INFO, "[PLAYER_COMMAND] " + talkingPlayer.getName() + ": " + msg);
        ...
        if (chat == ChatMode.FACTION) {
            ...
            Server.getInstance().getLogger().log(LogLevel.INFO, TextFormat.clean("FactionChat " + myFaction.getTag() + ": " + message));
            ...
            String message = String.format(Conf.allianceChatFormat, TextFormat.clean(me.getNameAndTag()), msg);
            ...
            Server.getInstance().getLogger().log(LogLevel.INFO, TextFormat.clean("AllianceChat: " + message));
        ...
        if (Conf.chatTagRelationColored) {
            ...
            for (CommandSender commandSender : event.getRecipients()) {
                if (commandSender instanceof Player) {
                    Player listeningPlayer = (Player) commandSender;
                    ...
                    try {
                        ...
                        P.p.log(LogLevel.CRITICAL, "Critical error in chat message formatting!");
                        P.p.log(LogLevel.CRITICAL, "NOTE: This has been automatically fixed right now by setting chatTagInsertIndex to 0.");
                        P.p.log(LogLevel.CRITICAL, "For a more proper fix, please read this regarding chat configuration: http://massivecraft.com/plugins/factions/config#Chat_configuration");
                        return;
                    }
                }
            }
            ...
            String nonColoredMsg = TextFormat.clean(String.format(nonColoredMsgFormat, talkingPlayer.getDisplayName(), msg));
            Server.getInstance().getLogger().log(LogLevel.INFO, nonColoredMsg);
        } else {
            // No relation color.
