...
public abstract class EntityCollection<E extends Entity> {
    ...
    private Map<String, E> loadCore() {
        ...
        try {
            ...
            Server.getInstance().getLogger().log(LogLevel.WARNING, "JSON error encountered loading \"" + file + "\": " + ex.getLocalizedMessage());

            // backup bad file, so user can attempt to recover something from it
            File backup = new File(file.getPath() + "_bad");
            if (backup.exists()) backup.delete();
            Server.getInstance().getLogger().log(LogLevel.WARNING, "Backing up copy of bad file to: " + backup);
