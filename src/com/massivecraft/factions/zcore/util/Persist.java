...
public class Persist {
    ...
    public <T> T loadOrSaveDefault(T def, Class<T> clazz, File file) {
        ...
        if (loaded == null) {
            p.log(LogLevel.WARNING, "Using default as I failed to load: " + file);

            // backup bad file, so user can attempt to recover their changes from it
            File backup = new File(file.getPath() + "_bad");
            if (backup.exists()) backup.delete();
            p.log(LogLevel.WARNING, "Backing up copy of bad file to: " + backup);
    ...
    public <T> T load(Class<T> clazz, File file) {
        ...
        try {
            ...
            p.log(LogLevel.WARNING, ex.getMessage());
        }
