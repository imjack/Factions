...
public abstract class SpiralTask implements Runnable {
    ...
    public SpiralTask(FLocation fLocation, int radius) {
        ...
        this.world = Server.getInstance().getLevelByName(fLocation.getWorldName());
        if (this.world == null) {
            P.p.log(LogLevel.WARNING, "[SpiralTask] A valid world must be specified!");
            this.stop();
            return;
        }
        ...
        this.setTaskID(Server.getInstance().getScheduler().scheduleRepeatingTask(this, 2).getTaskId());
    }
    ...
    public final Location currentLocation() {
        return new Location(FLocation.chunkToBlock(x), 65, FLocation.chunkToBlock(z), 0, 0, world);
    }
